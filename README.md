...
- **[devenv-doctor](./devenv-doctor)** — the plugin bundled in this repo. Type "check my setup" in Claude Code and it runs the real checks, then tells you exactly what to fix
- **Docker Desktop**, including WSL2 setup on Windows if it's missing
- **VS Code** + a starter extension pack: [Claude Code](https://marketplace.visualstudio.com/items?itemName=anthropic.claude-code), GitLens, Docker, ESLint, Prettier
...
Already have Claude Code and only want the plugin? Get the repo (git clone or ZIP, either works). Then, with a terminal open **inside that folder** (the one with `setup.sh` in it), start Claude Code:

```
claude
```

...and inside that session run:

```
/plugin marketplace add .
/plugin install devenv-doctor
```
